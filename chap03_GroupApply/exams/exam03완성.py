...
election = pd.read_csv('C://ITWILL//4_Python-ll//data//data//election_2012.csv', encoding='ms949')
print(election.info())
'''
cand_id : 대선 후보자 id
cand_nm : 대선 후보자 이름
...
#문) 두 후보자 모두 200만 달러 이상의 후원금을 지불한 직업군 필터일때
import numpy as np

# 논리식, 관계식 
over_2mn_each=ptable[np.logical_and(
    ptable['Obama, Barack']>=2000000, 
    ptable['Romney, Mitt']>=2000000)]

over_2mn_each
over_2mn_each.shape #(3, 2)
'''
cand_nm            Obama, Barack  Romney, Mitt
contbr_occupation                             
ATTORNEY             11126932.97    5302578.82
...
#문) 두 후보자 중 한 명이 200백만달러 이상 직업군
over_2mn_each2=ptable[np.logical_or(
    ptable['Obama, Barack']>=2000000, 
    ptable['Romney, Mitt']>=2000000)]
over_2mn_each2
'''cand_nm                                 Obama, Barack  Romney, Mitt
