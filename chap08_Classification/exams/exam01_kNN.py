...
def classify(know, not_know, cate, k ) :  #k=3실행수=>class_result = knn_classify(know, not_know, cate )
    #유클리드 거리계산식
...
x= int(input('단맛입력(1~10) : '))
y= int(input('아삭 거림 입력(1~10) : '))
k= int(input('k값 입력(1 or 3) : '))

not_know=np.array([x,y])

class_result = classify(know, not_know, cate)
print('-'*50)
print('classCount:', clas_result)
print('분류 결과 : ', max(class_result))
print('-'*40)
